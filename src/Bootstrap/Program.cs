...
if (!needCreateToken)
{
    ...
    try
    {
        var valid = await ValidateTeamCityTokenAsync(httpClient, teamcityUrl, existingTeamcityToken);
        if (!valid)
        {
...
static async Task<string?> TryCreateTeamCityTokenViaApiAsync(HttpClient client, string teamcityUrl, string username, string password, string tokenName)
{
    ...
    // Candidate endpoints (TeamCity REST has varied between versions)
    var endpoints = new[]
    {
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        foreach (var url in endpoints)
        {
            try
            {
                // Try XML payload first (TeamCity commonly expects XML)
                ...
                xmlReq.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", auth);
                xmlReq.Headers.Accept.Clear();
                xmlReq.Headers.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
                ...
                if (resp.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        if (tokenElem != null)
                        {
                            // look for attribute 'value' or 'tokenValue' or inner text
                    ...
                    // Fallback: if response body long enough, return it
                    if (!string.IsNullOrWhiteSpace(respText) && respText.Length > 20)
                        return respText.Trim();
                }

                // If not success, also try JSON payload variant
                ...
                jsonReq.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", auth);
                jsonReq.Headers.Accept.Clear();
                jsonReq.Headers.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
